...
def _project_root() -> Path | None:
    """定位项目根（含 .playwright-browsers / sau_cli.py 的目录），用于支持包内 tools/biliup。"""
    cur = Path(__file__).resolve()
    for p in [cur, *cur.parents]:
        if (p / ".playwright-browsers").is_dir() or (p / "sau_cli.py").exists():
            return p
    return None


def get_biliup_runtime_root() -> Path:
    # 优先使用包内 tools/biliup（离线包已内置，免联网下载）；否则退回用户目录缓存
    proj = _project_root()
    if proj is not None:
        bundled = proj / "tools" / "biliup"
        if bundled.is_dir():
            return bundled
