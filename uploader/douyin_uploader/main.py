...
async def douyin_cookie_gen(
    ...
    async with async_playwright() as playwright:
        if cdp_url:
            ...
            context = browser.contexts[0] if browser.contexts else await browser.new_context(timezone_id="Asia/Shanghai")
...
class DouYinBaseUploader(BaseVideoUploader):
    ...
    async def set_schedule_time_douyin(self, page, publish_date):
        ...
        await asyncio.sleep(1)
        target = publish_date.strftime("%Y-%m-%d %H:%M")
        input_loc = page.locator('.semi-input[placeholder="日期和时间"]')

        # 用 fill() 原子写入，避免逐字符 keyboard.type 触发 Semi 日期时间输入掩码把时间解析错位
        # （旧实现 keyboard.type 会导致抖音定时时间稳定偏移到设定值的 +2 小时）
        await input_loc.click()
        await input_loc.fill(target)
        await asyncio.sleep(0.5)
        await page.keyboard.press("Enter")
        await asyncio.sleep(1)

        # 回读校验：确认组件真正接受的是设定时间，而非被掩码/默认处理成其它时间
        got = (await input_loc.input_value()).strip()
        if got and got != target:
            douyin_logger.warning(_msg("⚠️", f"定时时间回读不一致: 期望 {target} 实际 {got}，改用键盘输入兜底"))
            await input_loc.click()
            ...
            await asyncio.sleep(1)
            got = (await input_loc.input_value()).strip()
        douyin_logger.info(_msg("🕒", f"抖音定时时间: 期望 {target} / 回读 {got}"))
    ...
    async def upload(self, playwright: Playwright) -> None:
        ...
        while True:
            try:
                ...
                # ── 干跑模式：仅校验定时时间设置是否正确，不真正发布 ──
                if os.environ.get("SAU_DRY_RUN_SCHEDULE") == "1":
                    sched_input = page.locator('.semi-input[placeholder="日期和时间"]')
                    val = (await sched_input.input_value()).strip() if await sched_input.count() else "(未设置定时)"
                    douyin_logger.success(_msg("🧪", f"[DRY-RUN] 定时时间回读={val}（未真正发布）"))
